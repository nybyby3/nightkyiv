...
cats:{concerts:"Concerts",culture:"Culture",nightlife:"Nightlife",food:"Food & Drinks",sports:"Sports",wellness:"Wellness",cinema:"Cinema",entertainment:"Entertainment",cooking:"Cook at Home",shopping:"Shopping",hotels:"Hotels",services:"Services"},
...
cats:{concerts:"Концерти",culture:"Культура",nightlife:"Нічне життя",food:"Їжа та напої",sports:"Спорт",wellness:"Відпочинок",cinema:"Кіно",entertainment:"Розваги",cooking:"Готуємо вдома",shopping:"Шопінг",hotels:"Готелі",services:"Сервіси"},
...
const IC={concerts:"🎵",culture:"🎨",nightlife:"🌙",food:"🍕",sports:"⚽",wellness:"✨",cinema:"🎬",entertainment:"🎮",cooking:"🍳",shopping:"🛍️",hotels:"🏨",services:"🔧"};
const CC={concerts:"#ff6bca",culture:"#3ecf8e",nightlife:"#a259ff",food:"#ff6b6b",sports:"#4fc3f7",wellness:"#36d1c4",cinema:"#f7b731",entertainment:"#ff9f43",cooking:"#ff8a50",shopping:"#e91e63",hotels:"#5c6bc0",services:"#78909c"};
...
const avCats=useMemo(()=>{const filtered=allVenues.filter(v=>(!grp||v.g.includes(grp)));const counts={};filtered.forEach(v=>{counts[v.c]=(counts[v.c]||0)+1;});return counts;},[allVenues,grp,tempo]);
const subResults=useMemo(()=>{if(!cat||!sc[cat])return{};const filtered=allVenues.filter(v=>(!grp||v.g.includes(grp))&&v.c===cat);const groups={};Object.entries(sc[cat]).forEach(([key,val])=>{const items=filtered.filter(v=>val.ids.includes(v.id));if(items.length>0)groups[key]={...val,items};});return groups;},[allVenues,grp,tempo,cat,sc]);
const results=useMemo(()=>{let filtered=allVenues.filter(v=>(!grp||v.g.includes(grp))&&v.c===cat);if(subcat&&sc[cat]&&sc[cat][subcat]){filtered=filtered.filter(v=>sc[cat][subcat].ids.includes(v.id));}return filtered.sort((a,b)=>{if(sortBy==="price")return(a.p-b.p)*dir;return a[lang].localeCompare(b[lang])*dir;});},[allVenues,grp,tempo,cat,subcat,sortBy,dir,lang,sc]);
...
if(step===2)return(<W><Hdr text={lang==="uk"?"\u042F\u043A\u0438\u0439 \u043D\u0430\u0441\u0442\u0440\u0456\u0439?":"What's the mood?"}/><Opt label={lang==="uk"?"\u0410\u043A\u0442\u0438\u0432\u043D\u0438\u0439":"Active"} icon={"⚡"} onClick={()=>{setTempo("active");next(3);}}/><Opt label={lang==="uk"?"\u0421\u043F\u043E\u043A\u0456\u0439\u043D\u0438\u0439":"Calm"} icon="🌿" onClick={()=>{setTempo("passive");next(3);}}/><Opt label={lang==="uk"?"\u041D\u0435 \u0432\u0430\u0436\u043B\u0438\u0432\u043E":"Doesn't matter"} icon="🤷" onClick={()=>{setTempo(null);next(3);}}/></W>);
...
const[dataLoaded,setDataLoaded]=useState(false);useEffect(()=>{fetch('/data.json').then(r=>r.json()).then(d=>{A=d.map((x,i)=>({id:i+1,en:x.e||x.n,uk:x.n,v:'',c:x.c,g:x.c,t:x.s,p:'',tm:x.h||'',s:x.s,addr:x.a||'',phone:x.p||''}));setDataLoaded(true)}).catch(()=>setDataLoaded(true))},[]);if(!dataLoaded)return(<div style={{display:'flex',justifyContent:'center',alignItems:'center',height:'100vh',color:'#fff',fontSize:'1.5em'}}>Loading...</div>);
