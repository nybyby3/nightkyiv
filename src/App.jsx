...
cats:{concerts:"Concerts",culture:"Culture",nightlife:"Nightlife",food:"Food & Drinks",sports:"Sports",cinema:"Cinema",entertainment:"Entertainment",cooking:"Cook at Home"},
...
cats:{concerts:"Концерти",culture:"Культура",nightlife:"Нічне життя",food:"Їжа та напої",sports:"Спорт",cinema:"Кіно",entertainment:"Розваги",cooking:"Готуємо вдома"},
...
const IC={concerts:"🎵",culture:"🎨",nightlife:"🌙",food:"🍕",sports:"⚽",cinema:"🎬",entertainment:"🎮",cooking:"🍳"};
const CC={concerts:"#ff6bca",culture:"#3ecf8e",nightlife:"#a259ff",food:"#ff6b6b",sports:"#4fc3f7",cinema:"#f7b731",entertainment:"#ff9f43",cooking:"#ff8a50"};
...
/* -- HOME PAGE -- */
function HomePage({lang,T:t,A:allVenues,CC:cc,IC:ic,SC:sc,setPage,userLoc}){
...
useEffect(()=>{if(navigator.geolocation)navigator.geolocation.getCurrentPosition(p=>setUserLoc([p.coords.latitude,p.coords.longitude]),()=>{},{enableHighAccuracy:true,timeout:10000});},[]);useEffect(()=>{fetch('/data.json').then(r=>r.json()).then(d=>{A=d.map((x,i)=>({id:i+1,en:x.e||x.n,uk:x.n,v:'',c:x.c,g:x.c,t:x.s,p:'',tm:x.h||'',s:x.s,addr:x.a||'',phone:x.p||'',lat:x.la,lng:x.lo,web:x.w||''}));setDataLoaded(true)}).catch(()=>setDataLoaded(true))},[]);if(!dataLoaded)return(<div style={{display:'flex',justifyContent:'center',alignItems:'center',height:'100vh',color:'#fff',fontSize:'1.5em'}}>Loading...</div>);
...
{page==="home"&&<HomePage lang={lang} T={T} A={A} CC={CC} IC={IC} SC={SC} setPage={setPage} userLoc={userLoc}/>}
