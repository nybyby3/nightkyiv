...
if(step===1)return(<W><Hdr text={lang==="uk"?"\u0425\u0442\u043E \u0439\u0434\u0435?":"Who's going?"}/><G><Opt label={lang==="uk"?"\u042F \u0441\u0430\u043C":"Solo"} icon="🙋" onClick={()=>{setGrp("solo");next(2);}}/><Opt label={lang==="uk"?"\u0412\u0434\u0432\u043E\u0445":"Together"} icon="👫" onClick={()=>{setGrp("duo");next(2);}}/><Opt label={lang==="uk"?"\u041A\u043E\u043C\u043F\u0430\u043D\u0456\u044F 3+":"Group 3+"} icon="🎉" onClick={()=>{setGrp("party");next(2);}}/><Opt label={lang==="uk"?"\u041D\u0435 \u0432\u0430\u0436\u043B\u0438\u0432\u043E":"Doesn't matter"} icon="🤷" onClick={()=>{setGrp(null);next(2);}}/></G></W>);
...
if(step===3){const cats=Object.entries(avCats).filter(([,n])=>n>0);return(<W><Hdr text={lang==="uk"?"\u0429\u043E \u0446\u0456\u043A\u0430\u0432\u0438\u0442\u044C?":"What interests you?"}/><G>{cats.map(([c,n])=><Opt key={c} label={(lang==="uk"?t[lang].cats[c]:c.charAt(0).toUpperCase()+c.slice(1))+" ("+n+")"} icon={ic[c]} color={cc[c]+"33"} onClick={()=>{if(c==="cinema"){setPage("cinema");return;}setCat(c);const subs=Object.keys(sc[c]||{});if(subs.length>0)next(4);else next(5);}}/>)}</G></W>);}
