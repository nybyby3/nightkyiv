...
const prev=useCallback(()=>{setAnimDir(-1);setStep(s=>{if(s===5)setSubcat(null);if(s===4)setCat(null);if(s===3)setTempo(null);if(s===2)setGrp(null);return Math.max(0,s-1)});},[]);
const next=useCallback((val,s)=>{setAnimDir(1);if(s===1)setGrp(val);if(s===2)setTempo(val);if(s===3)setCat(val);if(s===4)setSubcat(val);setStep(s);},[]);
const reset=useCallback(()=>{setAnimDir(-1);setStep(0);setGrp(null);setTempo(null);setCat(null);setSubcat(null);setSelVenue(null);},[]);
const avCats=useMemo(()=>{let f=allVenues;if(grp)f=f.filter(a=>a.g.includes(grp));if(tempo)f=f.filter(a=>a.t===tempo);const cs={};f.forEach(a=>{cs[a.c]=(cs[a.c]||0)+1});return cs;},[grp,tempo,allVenues]);
const results=useMemo(()=>{let f=allVenues;if(grp)f=f.filter(a=>a.g.includes(grp));if(tempo)f=f.filter(a=>a.t===tempo);if(cat)f=f.filter(a=>a.c===cat);if(subcat&&sc[cat]&&sc[cat][subcat])f=f.filter(a=>sc[cat][subcat].ids.includes(a.id));if(sortBy==="price")f=[...f].sort((a,b)=>(a.p-b.p)*dir);else if(sortBy==="name")f=[...f].sort((a,b)=>a[lang].localeCompare(b[lang])*dir);return f;},[grp,tempo,cat,subcat,allVenues,sc,sortBy,dir,lang]);
const subResults=useMemo(()=>{if(!cat||!sc[cat])return{};let f=allVenues;if(grp)f=f.filter(a=>a.g.includes(grp));if(tempo)f=f.filter(a=>a.t===tempo);f=f.filter(a=>a.c===cat);const sr={};Object.entries(sc[cat]).forEach(([k,v])=>{const cnt=f.filter(a=>v.ids.includes(a.id)).length;if(cnt>0)sr[k]={...v,count:cnt};});return sr;},[cat,sc,allVenues,grp,tempo]);
const slideStyle={animation:step>0?`slide${animDir>0?"In":"Out"} 0.3s ease`:"none"};
if(selVenue){const v=selVenue;return(<div style={{minHeight:"100vh",background:"#0d1117",padding:"20px 16px",color:"#e6edf3"}}><button onClick={()=>setSelVenue(null)} style={{background:"none",border:"none",color:"#a78bfa",fontSize:16,cursor:"pointer",marginBottom:16}}>â {lang==="uk"?"ÐÐ°Ð·Ð°Ð´":"Back"}</button><div style={{borderRadius:16,overflow:"hidden",background:"#161b22"}}>{v.img&&<div style={{height:200,background:`url(${v.img}) center/cover`,borderRadius:"16px 16px 0 0"}}/>}<div style={{padding:20}}><h2 style={{margin:"0 0 8px",fontSize:22}}>{v[lang]}</h2><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}><span style={{background:cc[v.c]+"22",color:cc[v.c],padding:"4px 10px",borderRadius:20,fontSize:13}}>{ic[v.c]} {t[lang].cats[v.c]}</span>{v.p>0&&<span style={{background:"#ffd74022",color:"#ffd740",padding:"4px 10px",borderRadius:20,fontSize:13}}>â´{v.p}</span>}{v.tm&&<span style={{background:"#40c4ff22",color:"#40c4ff",padding:"4px 10px",borderRadius:20,fontSize:13}}>â° {v.tm}</span>}{v.s&&<span style={{background:"#1de9b622",color:"#1de9b6",padding:"4px 10px",borderRadius:20,fontSize:13}}>ð {v.s}</span>}</div><p style={{color:"#8b949e",margin:"12px 0",lineHeight:1.6}}>{v.v}</p><div style={{display:"flex",gap:8,marginTop:16}}><button onClick={()=>{setPage("map")}} style={{flex:1,padding:"12px",borderRadius:12,background:"#a78bfa",color:"#fff",border:"none",fontSize:15,cursor:"pointer"}}>ð {lang==="uk"?"ÐÐ° ÐºÐ°ÑÑÑ":"On Map"}</button></div></div></div></div>);}
const W=({children,s})=>(<div key={s} style={{...slideStyle,padding:"0 16px"}}>{children}</div>);
const Hdr=({title,sub})=>(<div style={{textAlign:"center",marginBottom:24}}>{step>0&&<button onClick={prev} style={{position:"absolute",left:16,top:80,background:"none",border:"none",color:"#a78bfa",fontSize:22,cursor:"pointer"}}>â</button>}<h2 style={{fontSize:26,margin:"0 0 6px",color:"#e6edf3"}}>{title}</h2>{sub&&<p style={{color:"#8b949e",margin:0,fontSize:14}}>{sub}</p>}</div>);
const Opt=({icon,label,desc,color,onClick})=>(<button onClick={onClick} style={{width:"100%",padding:"16px 20px",borderRadius:14,background:"#161b22",border:"1px solid #30363d",color:"#e6edf3",cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:14,transition:"all 0.2s"}} onMouseOver={e=>{e.currentTarget.style.borderColor=color||"#a78bfa";e.currentTarget.style.transform="scale(1.02)"}} onMouseOut={e=>{e.currentTarget.style.borderColor="#30363d";e.currentTarget.style.transform="scale(1)"}}><span style={{fontSize:28}}>{icon}</span><div><div style={{fontSize:16,fontWeight:600}}>{label}</div>{desc&&<div style={{fontSize:13,color:"#8b949e",marginTop:2}}>{desc}</div>}</div></button>);
return(<div style={{minHeight:"100vh",background:"#0d1117",paddingTop:70,paddingBottom:80,position:"relative"}}>
<style>{`@keyframes slideIn{from{opacity:0;transform:translateX(60px)}to{opacity:1;transform:translateX(0)}}@keyframes slideOut{from{opacity:0;transform:translateX(-60px)}to{opacity:1;transform:translateX(0)}}`}</style>
{step===0&&<W s={0}><div style={{textAlign:"center",paddingTop:40}}><div style={{fontSize:14,color:"#8b949e",marginBottom:8}}>{t[lang].greeting()}</div><h1 style={{fontSize:32,margin:"0 0 12px",color:"#e6edf3"}}>{lang==="uk"?"Ð¯ÐºÐ¸Ð¹ Ð¿Ð»Ð°Ð½\nÐ½Ð° Ð²ÐµÑÑÑ?":"What's the plan\nfor tonight?"}</h1><p style={{color:"#8b949e",margin:"0 0 32px",fontSize:15}}>{allVenues.length} {lang==="uk"?"Ð°ÐºÑÐ¸Ð²Ð½Ð¾ÑÑÐµÐ¹ Ñ ÐÐ¸ÑÐ²Ñ â Ð·Ð½Ð°Ð¹Ð´Ð¸ ÑÐ²Ð¾Ñ":"activities in Kyiv â find yours"}</p><button onClick={()=>{setAnimDir(1);setStep(1)}} style={{width:"100%",padding:"18px",borderRadius:16,background:"linear-gradient(135deg,#a78bfa,#7c3aed)",color:"#fff",border:"none",fontSize:17,fontWeight:600,cursor:"pointer",boxShadow:"0 4px 20px #7c3aed44"}}>{lang==="uk"?"ÐÐ½Ð°Ð¹Ð´ÐµÐ¼Ð¾ ÑÐ¾ÑÑ â":"Let's find something â"}</button></div></W>}
{step===1&&<W s={1}><Hdr title={lang==="uk"?"Ð ÐºÐ¸Ð¼ ÑÐ´ÐµÑ?":"Who are you going with?"} sub={lang==="uk"?"Ð¦Ðµ Ð´Ð¾Ð¿Ð¾Ð¼Ð¾Ð¶Ðµ Ð¿ÑÐ´ÑÐ±ÑÐ°ÑÐ¸ Ð²Ð°ÑÑÐ°Ð½ÑÐ¸":"This helps us find the right options"}/><div style={{display:"flex",flexDirection:"column",gap:10}}><Opt icon="ð" label={lang==="uk"?"Ð¡Ð°Ð¼/Ð¡Ð°Ð¼Ð°":"Solo"} desc={lang==="uk"?"Ð§Ð°Ñ Ð´Ð»Ñ ÑÐµÐ±Ðµ":"Me time"} onClick={()=>next("solo",2)}/><Opt icon="ð«" label={lang==="uk"?"Ð Ð´ÑÑÐ³Ð¾Ð¼":"With a friend"} desc={lang==="uk"?"ÐÐ´Ð²Ð¾Ñ Ð·Ð°Ð²Ð¶Ð´Ð¸ ÐºÑÐ°ÑÐµ":"Better together"} onClick={()=>next("duo",2)}/><Opt icon="ð" label={lang==="uk"?"ÐÐ¾Ð¼Ð¿Ð°Ð½ÑÑ 3+":"Group 3+"} desc={lang==="uk"?"Ð§Ð¸Ð¼ Ð±ÑÐ»ÑÑÐµ ÑÐ¸Ð¼ Ð²ÐµÑÐµÐ»ÑÑÐµ":"The more the merrier"} onClick={()=>next("party",2)}/><Opt icon="ð¤·" label={lang==="uk"?"ÐÐµ Ð²Ð°Ð¶Ð»Ð¸Ð²Ð¾":"Doesn't matter"} color="#ffd740" onClick={()=>next(null,2)}/></div></W>}
{step===2&&<W s={2}><Hdr title={lang==="uk"?"Ð¯ÐºÐ¸Ð¹ Ð½Ð°ÑÑÑÑÐ¹?":"What's the mood?"} sub={lang==="uk"?"ÐÐºÑÐ¸Ð²Ð½Ð¾ ÑÐ¸ ÑÐ¿Ð¾ÐºÑÐ¹Ð½Ð¾?":"Active or chill?"}/><div style={{display:"flex",flexDirection:"column",gap:10}}><Opt icon="ð" label={lang==="uk"?"ÐÐºÑÐ¸Ð²Ð½Ð¸Ð¹":"Active"} desc={lang==="uk"?"Ð ÑÑ, Ð°Ð´ÑÐµÐ½Ð°Ð»ÑÐ½, ÑÐ¿Ð¾ÑÑ":"Movement, adrenaline, sports"} color="#40c4ff" onClick={()=>next("active",3)}/><Opt icon="ð" label={lang==="uk"?"Ð¡Ð¿Ð¾ÐºÑÐ¹Ð½Ð¸Ð¹":"Calm"} desc={lang==="uk"?"ÐÑÐ´Ð¿Ð¾ÑÐ¸Ð½Ð¾Ðº, ÐºÑÐ»ÑÑÑÑÐ°, ÑÐ¶Ð°":"Relaxation, culture, food"} color="#1de9b6" onClick={()=>next("passive",3)}/><Opt icon="ð¤·" label={lang==="uk"?"ÐÐµ Ð²Ð°Ð¶Ð»Ð¸Ð²Ð¾":"Doesn't matter"} color="#ffd740" onClick={()=>next(null,3)}/></div></W>}
{step===3&&<W s={3}><Hdr title={lang==="uk"?"Ð©Ð¾ ÑÑÐºÐ°Ð²Ð¸ÑÑ?":"What interests you?"} sub={lang==="uk"?"ÐÐ±ÐµÑÐ¸ ÐºÐ°ÑÐµÐ³Ð¾ÑÑÑ":"Choose a category"}/><div style={{display:"flex",flexDirection:"column",gap:10}}>{Object.entries(avCats).map(([c,cnt])=>(<Opt key={c} icon={ic[c]} label={t[lang].cats[c]} desc={cnt+" "+(lang==="uk"?"Ð²Ð°ÑÑÐ°Ð½ÑÑÐ²":"options")} color={cc[c]} onClick={()=>next(c,4)}/>))}</div></W>}
{step===4&&<W s={4}><Hdr title={lang==="uk"?"ÐÐ±ÐµÑÐ¸ Ð½Ð°Ð¿ÑÑÐ¼Ð¾Ðº":"Choose direction"} sub={cat&&t[lang].cats[cat]}/><div style={{display:"flex",flexDirection:"column",gap:10}}>{Object.entries(subResults).map(([k,v])=>(<Opt key={k} icon={v.ic} label={v[lang==="uk"?"uk":"en"]} desc={v.count+" "+(lang==="uk"?"Ð¼ÑÑÑÑ":"places")} color={cc[cat]} onClick={()=>next(k,5)}/>))}<Opt icon="ð" label={lang==="uk"?"ÐÐ¾ÐºÐ°Ð·Ð°ÑÐ¸ Ð²ÑÐµ":"Show all"} desc={Object.values(subResults).reduce((s,v)=>s+v.count,0)+" "+(lang==="uk"?"Ð¼ÑÑÑÑ":"places")} color="#ffd740" onClick={()=>next(null,5)}/></div></W>}
{step===5&&<W s={5}><Hdr title={lang==="uk"?"Ð ÐµÐ·ÑÐ»ÑÑÐ°ÑÐ¸":"Results"} sub={results.length+" "+(lang==="uk"?"Ð·Ð½Ð°Ð¹Ð´ÐµÐ½Ð¾":"found")}/><div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}><button onClick={()=>{setSortBy("name");setDir(1)}} style={{padding:"8px 14px",borderRadius:20,background:sortBy==="name"?"#a78bfa":"#161b22",color:sortBy==="name"?"#fff":"#8b949e",border:"1px solid "+(sortBy==="name"?"#a78bfa":"#30363d"),fontSize:13,cursor:"pointer"}}>{lang==="uk"?"ÐÐ° Ð½Ð°Ð·Ð²Ð¾Ñ":"By name"}</button><button onClick={()=>{setSortBy("price");setDir(d=>sortBy==="price"?-d:1)}} style={{padding:"8px 14px",borderRadius:20,background:sortBy==="price"?"#a78bfa":"#161b22",color:sortBy==="price"?"#fff":"#8b949e",border:"1px solid "+(sortBy==="price"?"#a78bfa":"#30363d"),fontSize:13,cursor:"pointer"}}>{lang==="uk"?"ÐÐ° ÑÑÐ½Ð¾Ñ":"By price"} {sortBy==="price"?(dir>0?"â":"â"):""}</button></div><div style={{display:"flex",flexDirection:"column",gap:10}}>{results.map(v=>(<button key={v.id} onClick={()=>setSelVenue(v)} style={{width:"100%",padding:"14px 16px",borderRadius:14,background:"#161b22",border:"1px solid #30363d",color:"#e6edf3",cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:12,transition:"all 0.2s"}} onMouseOver={e=>{e.currentTarget.style.borderColor=cc[v.c]}} onMouseOut={e=>{e.currentTarget.style.borderColor="#30363d"}}><span style={{fontSize:24}}>{ic[v.c]}</span><div style={{flex:1}}><div style={{fontSize:15,fontWeight:600}}>{v[lang]}</div><div style={{fontSize:12,color:"#8b949e",marginTop:2}}>{v.v}</div></div><div style={{textAlign:"right"}}>{v.p>0&&<div style={{color:"#ffd740",fontSize:14,fontWeight:600}}>â´{v.p}</div>}{v.tm&&<div style={{color:"#8b949e",fontSize:11}}>{v.tm}</div>}</div></button>))}</div>{results.length===0&&<div style={{textAlign:"center",padding:40,color:"#8b949e"}}><div style={{fontSize:48,marginBottom:12}}>ð¤</div><p>{lang==="uk"?"ÐÑÑÐ¾Ð³Ð¾ Ð½Ðµ Ð·Ð½Ð°Ð¹Ð´ÐµÐ½Ð¾. Ð¡Ð¿ÑÐ¾Ð±ÑÐ¹ ÑÐ½ÑÑ ÑÑÐ»ÑÑÑÐ¸":"Nothing found. Try different filters"}</p></div>}</W>}
{step>0&&<div style={{position:"fixed",bottom:90,left:"50%",transform:"translateX(-50%)",zIndex:10}}><button onClick={reset} style={{padding:"10px 24px",borderRadius:24,background:"#21262d",color:"#8b949e",border:"1px solid #30363d",fontSize:13,cursor:"pointer"}}>âº {lang==="uk"?"ÐÐ¾ÑÐ°ÑÐ¸ ÑÐ¿Ð¾ÑÐ°ÑÐºÑ":"Start over"}</button></div>}
</div>);
}
